...
export default function CustomerDetailTable({ data = [] }) {
  const [isOpenViewCustomerDetailModal, setIsOpenViewCustomerDetailModal] =
    useState(false);
        ...
        <tbody className="divide-y divide-gray-100">
          {data.map((customer, index) => (
            <tr key={index} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="px-6 py-4 text-gray-600">{customer.emailId}</td>
              ...
              <td className="px-6 py-4 text-center">
                <button
                  className="hover:text-brand-600 transition flex items-center gap-2 px-3 py-1.5 rounded-md text-sm text-brand-600 border border-brand-600 hover:bg-brand-50 cursor-pointer mx-auto"
